...
fn task_remove(stdout: &mut StandardStream, tasks: &mut todo::TaskVec, conf: &conf::Conf) -> io::Result<()> {
    ...
    if todos.is_empty() {
        ...
        if !flt_conf.dry {
            ...
            if calculate_updated(&removed) != 0 && let Err(e) = todo::save(tasks, Path::new(&flt_conf.todo_file)) {
                writeln!(stdout, "Failed to save to '{0:?}': {e}", &flt_conf.todo_file)?;
                std::process::exit(1);
            }
        }
...
fn task_clean(stdout: &mut StandardStream, tasks: &mut todo::TaskVec, conf: &conf::Conf) -> io::Result<()> {
    ...
    if todos.is_empty() {
        ...
        if !conf.dry {
            ...
            if !conf.wipe && let Err(e) = todo::archive(&cloned, &conf.done_file) {
                eprintln!("{e:?}");
                exit(1);
            }
            let removed = todo::remove(tasks, Some(&todos));
            if calculate_updated(&removed) != 0 && let Err(e) = todo::save(tasks, Path::new(&conf.todo_file)) {
                writeln!(stdout, "Failed to save to '{0:?}': {e}", &conf.todo_file)?;
                std::process::exit(1);
            }
        }
...
fn main() {
    ...
    if let Err(e) = err && e.kind() != io::ErrorKind::BrokenPipe {
        eprintln!("{e}");
        std::process::exit(1);
    }
}
