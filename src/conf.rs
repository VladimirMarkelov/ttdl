...
impl Conf {
    ...
    pub fn editor(&self) -> Option<PathBuf> {
        ...
        if spth.is_empty() && let Some(p) = &self.editor_path {
            spth = p.clone();
        }
...
fn parse_fmt(matches: &Matches, c: &mut fmt::Conf) {
    ...
    if let Some(s) = matches.opt_str("human") && !s.is_empty() {
        for f in s.split(',') {
    ...
    if let Some(s) = matches.opt_str("width") && let Ok(n) = s.parse::<u16>() {
        c.width = n;
    }
    ...
    if !c.atty && c.width == 0 {
        ...
        // TODO: sane limit?
        if (c.width > 2000 || c.width < 20) && let Some((w, _)) = term_size::dimensions() {
...
fn detect_filenames(matches: &Matches, conf: &mut Conf) {
    if let Ok(val) = env::var(TODOFILE_VAR) && !val.is_empty() {
        conf.todo_file = PathBuf::from(val);
    }

    if matches.opt_present("local") {
        conf.todo_file = PathBuf::from(TODO_FILE);
    } else if let Some(val) = matches.opt_str("todo-file") && !val.is_empty() {
        conf.todo_file = PathBuf::from(val);
    }
    ...
    if let Some(val) = matches.opt_str("done-file") && !val.is_empty() {
...
fn resolve_home_directory(path: &mut PathBuf) {
    if let Ok(path_striped) = path.strip_prefix("~") && let Some(home) = dirs::home_dir() {
...
fn update_colors_from_conf(tc: &tml::Conf, conf: &mut Conf) -> Result<()> {
    ...
    if conf.fmt.color_term == fmt::TermColorType::Auto && let Some(cs) = &tc.colors.color_term {
...
fn update_ranges_from_conf(tc: &tml::Conf, conf: &mut Conf) {
    if let Some(imp) = &tc.ranges.important && imp.len() == 1 {
    ...
    if let Some(ref old) = tc.ranges.old && let Ok(r) = todotxt::Recurrence::from_str(old) {
...
fn update_global_from_conf(tc: &tml::Conf, conf: &mut Conf) {
    if let Some(fname) = &tc.global.filename && !fname.is_empty() {
    ...
    if let Some(sort_fields) = &tc.global.sort && conf.sort.fields.is_none() {
        conf.sort.fields = Some(sort_fields.to_owned());
    }

    if let Some(sh) = &tc.global.shell && !sh.is_empty() {
